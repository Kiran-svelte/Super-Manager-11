"""
SambaNova Provider - Free OpenAI-compatible API with fast inference
Free tier with DeepSeek, Llama, Qwen models
"""
import os
import time
...
class SambaNovaProvider(BaseAIProvider):
    ...
    def __init__(self):
        ...
        if self.api_key and OPENAI_SDK_AVAILABLE:
            ...
            if not OPENAI_SDK_AVAILABLE:
                ...
                self._last_error = "SAMBANOVA_API_KEY not set"
    ...
    async def generate(
        ...
        **kwargs,
        ...
        if not self._client:
            raise Exception("SambaNova client not initialized")
    ...
    async def generate_embedding(
        ...
        raise NotImplementedError("SambaNova embedding support is limited. Use Ollama or OpenAI.")


_sambanova_provider: Optional[SambaNovaProvider] = None


def get_sambanova_provider() -> SambaNovaProvider:
    global _sambanova_provider
    if _sambanova_provider is None:
        _sambanova_provider = SambaNovaProvider()
    return _sambanova_provider
